...
// Avoid importing next/navigation here to prevent type-resolution errors in some setups.
// @ts-ignore: CSS module types may be missing in some TS configs
...
const CustomPage: React.FC = () => {
  // Use direct navigation to avoid depending on `next/navigation` types in this file.
  const { t } = useLanguage();
  ...
  // Load persisted products from localStorage and listen for storage events
  useEffect(() => {
    let mounted = true;
  ...
  const handleProductClick = (id: number) => { window.location.href = `/products/${id}`; };
