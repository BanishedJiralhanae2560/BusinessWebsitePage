...
const ALL_PRODUCTS: CatalogProduct[] = [
  ...
  { id: 7,  name: 'Product Name', category: 'Development Kits',             price: 0.00, originalPrice: 0.00, rating: 0, reviewCount: 0, badge: 'NEW',       inStock: true,  description: 'Short product description goes here. Highlight key features or benefits in one to two sentences.' },
  { id: 8,  name: 'Product Name', category: 'Development Kits',             price: 0.00, originalPrice: 0.00, rating: 0, reviewCount: 0, badge: undefined,   inStock: true,  description: 'Short product description goes here. Highlight key features or benefits in one to two sentences.' },
  { id: 9,  name: 'Product Name', category: 'Development Kits',             price: 0.00, originalPrice: 0.00, rating: 0, reviewCount: 0, badge: undefined,   inStock: false, description: 'Short product description goes here. Highlight key features or benefits in one to two sentences.' },
  ...
  return (
    <article className={`${styles.card} ${!product.inStock ? styles.cardOutOfStock : ''}`}>
      <div className={styles.cardImg} onClick={onClick}>
        ...
        {!product.inStock && (
          <div className={styles.outOfStockOverlay}>{t['catalog.out_of_stock'] || 'Out of Stock'}</div>
        )}
      </div>
      ...
      <div className={styles.cardBody}>
        ...
        <div className={styles.cardFooter}>
          <div className={styles.priceGroup}>
            <span className={styles.price}>{product.price > 0 ? `$${product.price.toFixed(2)}` : '$—'}</span>
            {hasDiscount && <span className={styles.priceOrig}>${product.originalPrice.toFixed(2)}</span>}
          </div>
          <button className={styles.viewBtn} onClick={onClick} disabled={!product.inStock}>
            {product.inStock ? (t['catalog.view'] || 'View →') : (t['catalog.unavailable'] || 'Unavailable')}
          </button>
        </div>
      </div>
    </article>
...
const DevKitsPage: React.FC = () => {
  const router = useRouter();
  const { t } = useLanguage();

  const [search,         setSearch]         = useState('');
  const [activeCategory, setActiveCategory] = useState('Development Kits');
    ...
    <div className={styles.page}>
      ...
      <div className={styles.hero}>
        <div className={styles.heroContent}>
          <p className={styles.heroEyebrow}>{t['catalog.eyebrow.dev_kits'] || '// DEVELOPMENT KITS'}</p>
          <h1 className={styles.heroTitle}>{t['catalog.title.dev_kits'] || 'Development Kits'}</h1>
          <p className={styles.heroSub}>{t['catalog.subtitle'] || 'Products coming soon. Placeholder listings are shown below — real products will appear here once available.'}</p>
        </div>
        <div className={styles.heroGrid} aria-hidden="true">
          {Array.from({ length: 64 }).map((_, i) => <div key={i} className={styles.heroGridCell} />)}
        </div>
      </div>
      ...
      <main className={styles.main}>
        {filtered.length > 0 ? (
          ...
          <div className={styles.empty}>
            ...
            <button className={styles.emptyReset} onClick={() => { setSearch(''); setActiveCategory('Development Kits'); setInStockOnly(false); }}>
